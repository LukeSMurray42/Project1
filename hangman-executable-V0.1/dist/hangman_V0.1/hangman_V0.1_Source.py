...
def random_word():
    words = [line.strip() for line in open('words.txt') if len(line) > 5] # Defines how long a word must be - we can change the difficulty here
...
class Game(db.Model):
    ...
    @property
    def lost(self):
        return len(self.errors) == 7 # LOSE CONDITION - Can change the difficulty here
